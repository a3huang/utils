...
def clean_comments(raw_comments):
    ...
    for comment in raw_comments:
        ...
        words = comment.split()
        comment = ' '.join([w for w in words if w not in nltk.corpus.stopwords.words("english")])
        #ps = nltk.stem.PorterStemmer()
        #comment = [ps.stem(word) for word in comment.split(" ")]
        comment = comment.split(" ")
        comment = " ".join(comment).encode('utf-8')
        comment = str(comment)
        comment = comment.decode('unicode_escape').encode('ascii','ignore') # remove \x stuff
...
def analyze_sentiment(cleaned_comments):
    ...
    analyzed_comments = pd.DataFrame({'clean': cleaned_comments, 'subjectivity': subjectivity, 'polarity': polarity})
...
def display_topics(model, vectorizer, docs_df, n_top_words, n_top_documents):
    ...
    for topic_idx, topic in enumerate(topic_word_matrix):
        ...
        top_doc_indices = np.argsort(doc_topic_matrix[:, topic_idx])[::-1][0:n_top_documents]

        prefix = '    * '
        wrapper = textwrap.TextWrapper(initial_indent=prefix, width=90, subsequent_indent=' '*len(prefix))
...
# twts['pol'] = twts['polarity'].apply(lambda x: 1 if x > 0 else x).apply(lambda x: -1 if x < 0 else x)
#
